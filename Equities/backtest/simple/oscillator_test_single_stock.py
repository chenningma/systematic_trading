...
qualify row_number() over (partition by symbol order by return_5m desc) = 1
order by return_5m desc
'''
query_job = client.query(query)
latest_results = query_job.result().to_dataframe()

old_results = old_results.merge(coin_list, how = 'inner', on = 'symbol')
