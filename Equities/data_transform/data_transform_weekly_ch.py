...
def create_output_table(output_table):
    client.command(f'DROP TABLE {output_table}')
    query = f"""
        ...
        ENGINE = MergeTree()
        ORDER BY (ticker, date_start,date_end, field )
        """
...
def insert_aggregate_avg_by_period(input_table,output_table,price_field, period, start_date, new_field_name=None):
    ...
    if period == 'WEEK':
        date_start_statement = 'toStartOfWeek(date, 1) AS date_start'
        date_end_statement = 'addDays(toStartOfWeek(date, 1), 4) AS date_end'
    elif period == 'MONTH':
        date_start_statement = 'toStartOfMonth(date) AS date_start'
...
def insert_aggregate_close_by_period(input_table, output_table,price_field, period, start_date, new_field_name=None):
    ...
    INSERT INTO {output_table} 
        SELECT
            ...
            {date_end_statement},
            '{fieldname}' AS field,
            argMax({price_field}, date) AS value
        FROM
           {input_table}
        WHERE
            date >= '{start_date}'
...
def pivot_table(input_table, output_table):
    fields = client.query_df(f'select distinct field from {input_table}').field.tolist()

    field_cases = ',\n        '.join([f"MAX(if(field = '{field}',  value, NULL)) AS {field}" for field in fields])
    query = f'''
        CREATE OR REPLACE VIEW {output_table} as
        SELECT
            ticker,
            date_start,
            date_end,
...
output_table = 'transformed.stage_weekly_levels'
...
def check_px_close():
    ...
    a.*,
    b.closeadj
    from transformed.stage_weekly_levels a
    left join raw.prices b on a.ticker= b.ticker and a.date_end = b.date
    where ticker = 'CVNA' and date_start > '2025-01-01' and field = 'px_close'
    '''
    client.command(query)
...
input_table = 'transformed.stage_weekly_levels'
