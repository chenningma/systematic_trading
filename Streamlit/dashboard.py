...
def get_ptf_constituents(schema, model_id, vars, get_date):
    ...
    if len(vars) == 5:
        ...
        left join prices.ticker_info_table t on ptf.ticker = t.symbol
...
Qualify row_number() over (partition by Ticker order by run_date desc) = 1
)
select 
o.Company_Name,
o.Ticker,
...
g.News,
g.run_date
from prod.openai_stock_summary o
left join gemini g on o.Ticker = g.Ticker
'''
query_job = client.query(query)
...
def ChngDt(wch_option):
    ...
    if wch_option == "3m":
        ...
        st.session_state.ksd = latest_date + pd.offsets.MonthEnd(-7)
    elif wch_option == "10m":
        st.session_state.ksd = latest_date + pd.offsets.MonthEnd(-10)
    elif wch_option == "YTD":
        st.session_state.ksd = latest_date + pd.offsets.YearEnd(-1)
    elif wch_option == "1Y":
        st.session_state.ksd = latest_date + pd.offsets.MonthEnd(-12)
    elif wch_option == "20m":
        st.session_state.ksd = latest_date + pd.offsets.MonthEnd(-20)
    elif wch_option == "2Y":
...
if button1.button("6m", on_click=ChngDt, args=("6m",)):
    wch_option = "6m:"

if button2.button("7m", on_click=ChngDt, args=("7m",)):
    wch_option = "7m:"

if button3.button("10m", on_click=ChngDt, args=("10m",)):
    wch_option = "10m:"

if button4.button("YTD", on_click=ChngDt, args=("YTD",)):
    wch_option = "YTD:"

if button5.button("1Y", on_click=ChngDt, args=("1Y",)):
    wch_option = "1Y:"

if button6.button("20m", on_click=ChngDt, args=("20m",)):
    wch_option = "20m:"

if button7.button("2Y", on_click=ChngDt, args=("2Y",)):
    wch_option = "2Y:"
